/**
 * AI 灵感创作工坊 V6 (Alpine.js 响应式重构 + 性能优化版) - Wasmer Edge JS Worker
 * 包含：弹簧动画、虚拟列表渲染、任务持久化与重试、多级触感反馈、本地数据库
 * 
 * 从 Cloudflare Worker 迁移至 Wasmer Edge：
 * - 进入点从 export default { fetch } 改为 addEventListener("fetch", ...)
 * - API 配置支持环境变量（Wasmer env / process.env）
 */

// ========== API 配置（支持环境变量覆盖） ==========
const API_BASE = (typeof process !== 'undefined' && process.env?.API_BASE) || "https://pucoding.com";
const API_KEYS = ((typeof process !== 'undefined' && process.env?.API_KEYS) || "sk-eae8e4ee6c612e45dba945ee66c347ee955b4f4b9d85baf0d945313f9291fc2e").split(",").map(s => s.trim()).filter(Boolean);
const MODEL_NAME = (typeof process !== 'undefined' && process.env?.MODEL_NAME) || "gpt-image-2";
...
// ========== Wasmer Edge JS Worker 進入點 ==========
// 使用 Service Worker 風格的 addEventListener 註冊 fetch 處理器
...
async function handleRequest(request) {
  ...
  // POST /api/generate → 代理至上游 /v1/images/generations
  ...
  // POST /api/edit → 代理至上游 /v1/images/edits
